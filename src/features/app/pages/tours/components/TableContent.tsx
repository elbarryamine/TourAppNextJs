...
import { Pagination, Rating, ModalMessage } from '../../../components'
...
export function TableContent({ tours, isSearching }: PropsTableContent) {
  ...
  const [selectedIdsOfTours, setSelectedIdsOfTours] = useState<string[]>([])
  const [selectedId, setSelectedId] = useState<string>('')
  const [runDeleteTours, { loading, data }] = useMutation(QUERYDELETESINGLE)
  const [pageCount, setPageCount] = useState<number>(0)
  const [itemOffset, setItemOffset] = useState<number>(0)
  const [displayedTours, setDisplayedTours] = useState<Tour[]>(tours.slice())
  ...
  return (
    <Box pos="relative">
      ...
      <Pagination
        initialPage={Number(qr.parse(search).page) ? Number(qr.parse(search).page) - 1 : 0}
        pageCount={pageCount}
...
function TourRow({ tour, handleChecked, selectedIdsOfTours, setSelectedId, setOpenModal }: TourRowProps) {
  ...
  return (
    <Grid pos="relative" _hover={{ bg: accenttext }} key={tour.id} py="10px" templateColumns={'repeat(4,1fr)'} placeItems="center">
      <Flex align="center" w="max-content" justify="flex-end">
        <Checkbox
          onChange={(value: ChangeEvent<HTMLInputElement>) => handleChecked(value, tour.id)}
          borderColor={text}
